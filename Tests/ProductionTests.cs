...
public class ProductionTests
{
    ...
    public async Task ProduceTextImageAsync()
    {
        ...
        var loadedimg = Dlib.LoadImage<RgbPixel>(tempFilePath);

        param1.Boxes = DataGeneration.BuildDefaultBoxes(loadedimg);
        var param2 = new ParamForTextCreation();

        // Act
        var prodinstance = new Production(logger);
        await prodinstance.ProduceTextPictures(picData, settings);
    }
    ...
    public async Task CreateImage_ValidFile_ReturnsMagickImage()
    {
        ...
        var logserviceOb = mockLogService.Object;
        var jobService = new JobService();
        var settings = new Settings();

        //Act'
        var prodInstance = new Production(logserviceOb);
        var result = await prodInstance.CreateImage(pictureData, settings);
    ...
    public async Task CreateImage_InvalidFile_LogsErrorAndReturnsNull()
    {
        ...
        var MockLoggerObj = mockLogService.Object;
        var prodInstance = new Production(MockLoggerObj);
