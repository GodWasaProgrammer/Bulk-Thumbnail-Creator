...
namespace Bulk_Thumbnail_Creator
{
	...
	{
		public static void RunFFMpeg(string filename, int scenedetection, string outputfolder)
		{
			string ExtractedFileName = Path.GetFileName(filename);
			outputfolder = Path.GetFullPath(outputfolder);
			
			string Parameters = $" -i " + $@"""{ExtractedFileName}"" " + "-vf " + $@"""select=gt(scene\,0.{scenedetection})\ """ + " -vsync vfr " + $@"""{outputfolder}/%03d.png""";
